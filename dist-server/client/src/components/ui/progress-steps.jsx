...
export function ProgressSteps({ currentStep }) {
    ...
    const getStepStatus = (stepId) => {
        if (stepId === 'uploaded')
            return 'completed';
        if (stepId === 'processing' && (currentStep === 'processing' || currentStep === 'completed'))
            return 'active';
        if (stepId === 'separating' && currentStep === 'completed')
            return 'completed';
        if (stepId === 'completed' && currentStep === 'completed')
            return 'completed';
        return 'waiting';
    };
    ...
    return (<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {steps.map((step) => {
            ...
            return (<div key={step.id} className="text-center">
            <div className={`w-12 h-12 ${styles.circle} rounded-full flex items-center justify-center mx-auto mb-2 transition-colors`}>
              {status === 'completed' ? (<CheckCircle className="w-6 h-6"/>) : status === 'active' ? (<Icon className="w-5 h-5 animate-spin"/>) : (<Icon className="w-5 h-5"/>)}
            </div>
