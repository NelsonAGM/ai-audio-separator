...
import { Music, AudioWaveform, HelpCircle, CheckCircle, Upload, Cog, Download, Plus, Mic, Drum, Guitar, Brain, Scissors, Headphones } from "lucide-react";
export default function Home() {
    ...
    const { data: audioFile, refetch } = useQuery({
    ...
    const handleDownloadTrack = (trackType) => {
        if (!audioFileId)
            return;
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Upload Section */}
        {currentSection === 'upload' && (<Card className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 mb-8">
            <CardContent className="p-0">
              <div className="text-center">
                ...
                {selectedFile && (<div className="mt-6 p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => setSelectedFile(null)} className="text-gray-400 hover:text-gray-600">
                        ×
                      </Button>
                    </div>
                  </div>)}

                {selectedFile && !audioFileId && (<Button onClick={handleUpload} disabled={uploadMutation.isPending} className="mt-6 bg-secondary text-white px-8 py-3 hover:bg-emerald-700">
                ...
                {audioFileId && audioFile?.status === 'uploaded' && (<Button onClick={handleStartSeparation} disabled={separateMutation.isPending} className="mt-6 bg-secondary text-white px-8 py-3 hover:bg-emerald-700">
        ...
        {/* Processing Section */}
        {currentSection === 'processing' && (<Card className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 mb-8">
            <CardContent className="p-0">
              <div className="text-center">
                ...
                <div className="mt-8 pt-6 border-t border-gray-200">
                  <Button onClick={handleResetProcessing} disabled={resetMutation.isPending} variant="outline" className="bg-red-50 text-red-600 border-red-200 hover:bg-red-100">
        ...
        {/* Results Section */}
        {currentSection === 'results' && audioFile?.tracks && (<Card className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
            <CardContent className="p-0">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                if (!track)
                    return null;
                return (<div key={config.type} className={`bg-gradient-to-br ${config.gradient} border ${config.border} rounded-xl p-6`}>
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleDownloadTrack(config.type)} className={`${config.textColor} ${config.hoverColor}`}>
                          <Download className="w-5 h-5"/>
                        </Button>
                      </div>
                      
                      <AudioPlayer audioFileId={audioFileId} trackType={config.type} config={config}/>
                    </div>);
            })}
              </div>

              <div className="text-center mt-8 pt-6 border-t border-gray-200">
                <Button className="bg-primary text-white px-8 py-3 hover:bg-blue-700 mr-4" onClick={() => {
                // Download all tracks
                ...
                <Button variant="outline" onClick={handleProcessNewFile} className="bg-gray-100 text-gray-700 px-6 py-3 hover:bg-gray-200">
